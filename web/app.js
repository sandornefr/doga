...
baglyok: {
  ...
  checks: [
    ...
    {
      id: "title",
      label: "3. Böngésző címsorában megjelenő cím \"Baglyok\"",
      check: (doc, html) => doc.title === "Baglyok" && _ch.inHead(html, /<title[^>]*>/i),
    },
    {
      id: "felirat-h1",
      label: "4. A \"Baglyok\" szöveg 1-es szintű címsor (h1#felirat)",
      check: (doc) => doc.querySelector('h1#felirat') !== null,
    },
    {
      id: "fejlec-img",
      label: "5. Az img mappában található fejlec.jpg képet elhelyezte",
    ...
    {
      id: "fejlec-id",
      label: "7. A fejléc képre egyedi azonosítót állított fejlec néven",
      check: (doc) => doc.querySelector('img#fejlec') !== null,
    },
    {
      id: "nav-3rd-link",
      ...
      check: (doc) => {
        const links = doc.querySelectorAll('nav a, .navbar a');
        return Array.from(links).some(a =>
          a.textContent.includes('Rejtőzk') && a.href.includes('erdekesseg.hu')
        );
      },
    },
    {
    ...
    },
    {
      ...
      label: "12. A Füleskuvik \"Különlegessége:\" szöveg aláhúzott",
      check: (doc) => {
    ...
    },
    {
      ...
      check: (doc) => {
        ...
        if (!uhuH2) return doc.querySelector('ul#taplalek li') !== null;
        const container = uhuH2.closest('.col');
        return container && container.querySelector('ul li') !== null;
      },
    },
    {
      id: "uhu-taplalek-id",
    ...
    {
      id: "hobagoly-h2",
      label: "16. A Hóbagoly részben van 2-es szintű címsor",
      check: (doc) => {
        const h2s = doc.querySelectorAll('h2');
    ...
    },
    {
      ...
      check: (doc) => {
        ...
        if (!hobH2) return false;
        const container = hobH2.closest('.col');
        if (!container) return false;
        return container.querySelectorAll('p:not(.card-text)').length >= 2;
      },
    },
    ...
    {
      id: "caption-classes",
      label: "20. A képaláírásokon alkalmazza a text-center és fw-bold osztályjelölőket (legalább 6)",
      check: (doc) => doc.querySelectorAll('p.fw-bold.text-center').length >= 6,
    },
    {
      id: "bold-combined",
      label: "21. A madárnevek félkövérek (legalább 6 helyen)",
      check: (doc) => doc.querySelectorAll('b, strong').length >= 6,
    },
    {
      id: "italic-combined",
      label: "22. A latin nevek dőltek (legalább 6 helyen)",
      check: (doc) => doc.querySelectorAll('i, em').length >= 6,
    },
    {
      id: "table-th-headers",
      label: "23. A táblázat fejlécében elkészítette a 6 fejléc cellát",
      check: (doc) => doc.querySelectorAll('thead th').length >= 6,
    },
    {
    ...
    },
    {
      ...
      check: (doc) => {
        ...
        return nokturn.querySelectorAll('[class*="col-md-6"]').length >= 2;
      },
    },
    {
      id: "footer-href",
    ...
    {
      id: "css-body-font",
      label: "28. CSS: Az oldal betűtípusa Rubik",
      check: (doc, html, css) => css && /body\s*\{[^}]*font-family\s*:\s*[^;]*rubik/i.test(css),
      cssCheck: true,
    },
    {
    ...
    {
      id: "css-felirat-smallcaps",
      label: "30. CSS: A felirat azonosítóra kiskapitális betű van beállítva",
      check: (doc, html, css) => css && /#felirat\s*\{[^}]*font-variant\s*:\s*small-caps/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-felirat-bold",
      label: "31. CSS: A felirat azonosítóra félkövér betűstílus van beállítva",
      check: (doc, html, css) => css && /#felirat\s*\{[^}]*font-weight\s*:\s*bold/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-nav-border",
      label: "32. CSS: A navigáció listaelemére 3px vastag, pontozott, narancssárga jobb szegélyt állított",
    ...
    {
      id: "css-bagoly-bg",
      label: "34. CSS: A bagoly osztály háttérszíne #f0edd1",
      check: (doc, html, css) => css && /\.bagoly\s*\{[^}]*background(-color)?\s*:\s*#f0edd1/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-lablec-color",
      label: "35. CSS: A lablec osztály hivatkozásának betűszíne fehér",
      check: (doc, html, css) => css && /\.lablec\s+a\s*\{[^}]*color\s*:\s*white/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-lablec-nodecor",
      label: "36. CSS: A lablec osztály hivatkozásánál nincs aláhúzás",
      check: (doc, html, css) => css && /\.lablec\s+a\s*\{[^}]*text-decoration\s*:\s*none/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-taplalek-image",
      label: "37. CSS: A taplalek azonosítóra bagoly.png listaelem kép van beállítva",
      check: (doc, html, css) => css && /#taplalek\s*\{[^}]*list-style-image\s*:\s*url\([^)]*bagoly\.png/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-p-justify",
      label: "38. CSS: A bekezdések sorkizártak",
      check: (doc, html, css) => css && /p\s*\{[^}]*text-align\s*:\s*justify/i.test(css),
      cssCheck: true,
    },
    {
...
egijelensegek: {
  ...
  checks: [
    ...
    {
      id: "title",
      label: "3. Böngésző címsorában megjelenő cím \"Égi jelenségek\"",
    ...
    {
      id: "felirat-h1",
      label: "7. Az \"Égi jelenségek\" szöveg 1-es szintű címsor (h1#felirat)",
      check: (doc) => doc.querySelector('h1#felirat') !== null,
    },
    {
      id: "nav-3rd-link",
    ...
    },
    {
      ...
      label: "11. A napfogyatkozás típusai számozatlan felsorolásban (ul, li elemekkel)",
      check: (doc) => doc.querySelector('ul#nap li') !== null,
    },
    {
      id: "napfogyatkozas-nap-id",
      label: "12. A napfogyatkozás ul egyedi azonosítója nap",
      check: (doc) => doc.querySelector('ul#nap') !== null,
    },
    {
      id: "verhold-ol",
      label: "13. A telihold ősi nevei számozott felsorolásban (ol, legalább 10 elem)",
      check: (doc) => {
    ...
    {
      id: "saskod-h2",
      label: "14. A Sas-köd részben van 2-es szintű címsor",
      check: (doc) => {
        const h2s = doc.querySelectorAll('h2');
    ...
    {
      id: "saskod-paragraphs",
      label: "15. A Sas-köd szövegrészei bekezdésbe kerültek (legalább 2 p)",
      check: (doc) => {
        const h2s = doc.querySelectorAll('h2');
        const saskodH2 = Array.from(h2s).find(h => h.textContent.includes('Sas-köd'));
        if (!saskodH2) return false;
        const container = saskodH2.closest('.col');
        if (!container) return false;
        return container.querySelectorAll('p:not(.card-text)').length >= 2;
      },
    },
    {
      id: "saskod-bold",
      label: "16. A Sas-köd neve félkövér",
      check: (doc) => {
        const bolds = doc.querySelectorAll('b, strong');
        return Array.from(bolds).some(b => b.textContent.includes('Sas-köd'));
      },
    },
    {
      id: "saskod-italic",
      label: "17. A Sas-köd alternatív neve (Messier) dőlt",
      check: (doc) => {
        const italics = doc.querySelectorAll('i, em');
        return Array.from(italics).some(i => i.textContent.includes('Messier'));
      },
    },
    {
      id: "saskod-img",
      label: "18. A Sas-köd kép forrása img/saskod.jpg",
      check: (doc) => doc.querySelector('img[src*="saskod"]') !== null,
    },
    {
      id: "saskod-alt-title",
      label: "19. A Sas-köd képnél alt és title szövege \"Sas-köd\"",
      check: (doc) => {
        const img = doc.querySelector('img[src*="saskod"]');
        return img && img.alt === "Sas-köd" && img.title === "Sas-köd";
      },
    },
    {
      id: "caption-classes",
      label: "20. A képaláírásokon alkalmazza a fw-bold és text-center osztályjelölőket (legalább 6)",
      check: (doc) => doc.querySelectorAll('p.fw-bold.text-center').length >= 6,
    },
    {
      id: "table-new-rows",
      label: "21. A táblázatba bekerültek az új sorok (Plútó, Xena)",
      check: (doc) => {
    ...
    {
      id: "table-bordered",
      label: "22. A táblázaton alkalmazza a table-bordered osztályjelölőt",
      check: (doc) => {
        const table = doc.querySelector('table');
        return table && table.classList.contains('table-bordered');
      },
    },
    {
      id: "table-th-w25",
      label: "23. A táblázat fejléc celláin alkalmazza a w-25 osztályjelölőt",
      check: (doc) => doc.querySelectorAll('thead th.w-25').length >= 3,
    },
    {
      id: "egyuttallas-col-md-6",
      label: "24. A Bolygó együttállás részben mindkét oszlop col-md-6 méretű",
      check: (doc) => {
        const egyuttallas = doc.querySelector('#egyuttallas');
        if (!egyuttallas) return false;
        return egyuttallas.querySelectorAll('[class*="col-md-6"]').length >= 2;
      },
    },
    {
      id: "footer-href",
      label: "25. A láblécben van hivatkozás a \"leiras\" azonosítóra",
      check: (doc) => doc.querySelector('.lablec a[href="#leiras"]') !== null,
    },
    {
      id: "footer-text",
      label: "26. A lábléc hivatkozás szövege \"Ugrás az elejére\"",
      check: (doc) => {
        const link = doc.querySelector('.lablec a');
        return link && link.textContent.includes('Ugrás az elejére');
      },
    ...
    {
      id: "css-body-font",
      label: "27. CSS: Az oldal betűtípusa IBM Plex Mono",
      check: (doc, html, css) => css && /body\s*\{[^}]*font-family\s*:\s*[^;]*IBM\s*Plex\s*Mono/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-fejlec-height",
      label: "28. CSS: A fejlec azonosítóra 100%-os magasság van beállítva",
      check: (doc, html, css) => css && /#fejlec\s*\{[^}]*height\s*:\s*100%/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-felirat-smallcaps",
      label: "29. CSS: A felirat azonosítóra kiskapitális betű van beállítva",
      check: (doc, html, css) => css && /#felirat\s*\{[^}]*font-variant\s*:\s*small-caps/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-felirat-color",
      label: "30. CSS: A felirat azonosító betűszíne rgb(13,1,81)",
      check: (doc, html, css) => css && /#felirat\s*\{[^}]*color\s*:\s*rgb\s*\(\s*13\s*,\s*1\s*,\s*81\s*\)/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-nav-border",
      label: "31. CSS: A navigáció listaelemére 3px vastag, pontozott, narancssárga jobb szegélyt állított",
      check: (doc, html, css) => css && /nav\s+li\s*\{[^}]*border(-right)?\s*:[^;]*3px[^;]*dotted[^;]*orange/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-nav-padding",
      label: "32. CSS: A navigáció listaelemének vízszintes belső margója 15px",
      check: (doc, html, css) => css && /nav\s+li\s*\{[^}]*padding\s*:\s*0\s*15px/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-hobbi-bg",
      label: "33. CSS: A hobbi osztály háttérszíne fehér (#ffffff)",
      check: (doc, html, css) => css && /\.hobbi\s*\{[^}]*background(-color)?\s*:\s*(#ffffff|white)/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-lablec-color",
      label: "34. CSS: A lablec osztály hivatkozásának betűszíne fehér",
      check: (doc, html, css) => css && /\.lablec\s+a\s*\{[^}]*color\s*:\s*white/i.test(css),
      cssCheck: true,
    },
    {
    ...
    {
      id: "css-navbar-fontsize",
      label: "36. CSS: A navbar osztály betűmérete 1.2em",
      check: (doc, html, css) => css && /\.navbar\s*\{[^}]*font-size\s*:\s*1\.2em/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-nap-image",
      label: "37. CSS: A nap azonosítóra nap.png listaelem kép van beállítva",
...
evmadarai: {
  ...
  checks: [
    ...
    {
      id: "title",
      label: "3. Böngésző címsorában megjelenő cím \"Év madarai\"",
      check: (doc, html) => doc.title === "Év madarai" && _ch.inHead(html, /<title[^>]*>/i),
    },
    {
      id: "fejlec-img",
      label: "4. A fejlec.jpg képet elhelyezte",
      check: (doc) => doc.querySelector('img[src*="fejlec"]') !== null,
    },
    {
      id: "fejlec-alt-title",
      label: "5. A fejléc kép alt és title szövege \"Év madarai\"",
      check: (doc) => {
        const img = doc.querySelector('img[src*="fejlec"]');
    ...
    {
      id: "felirat-h1",
      label: "7. Az \"Év madarai\" szöveg 1-es szintű címsor (h1#felirat)",
      check: (doc) => doc.querySelector('h1#felirat') !== null,
    },
    {
      id: "nav-3rd-link",
      label: "8. Elkészítette a \"Magyarország madarai\" menüpontot és jó oldalra hivatkozik",
      check: (doc) => {
        const links = doc.querySelectorAll('nav a, .navbar a');
        return Array.from(links).some(a =>
    ...
    {
      id: "nav-3rd-blank",
      label: "9. A \"Magyarország madarai\" link új lapon nyílik meg",
      check: (doc) => {
        const links = doc.querySelectorAll('nav a, .navbar a');
    ...
    {
      id: "vandorsolyom-paragraphs",
      label: "10. A Vándorsólyom szövegrészei bekezdésbe kerültek (legalább 2 p)",
      check: (doc) => {
        const h2s = doc.querySelectorAll('h2');
        const vandH2 = Array.from(h2s).find(h => h.textContent.includes('Vándorsólyom'));
        if (!vandH2) return false;
        const container = vandH2.closest('.col');
        if (!container) return false;
        return container.querySelectorAll('p:not(.card-text)').length >= 2;
      },
    },
    {
      ...
      label: "11. A Gólyatöcs \"Jellemvonásaik:\" szöveg aláhúzott",
      check: (doc) => {
        const underlines = doc.querySelectorAll('u');
    ...
    {
      id: "fulesbagoly-ol",
      label: "12. Az Erdei fülesbagoly megjelenésének adatai számozott felsorolásban (ol)",
      check: (doc) => {
        const h2s = doc.querySelectorAll('h2');
    ...
    {
      id: "ciganycsuk-ul",
      label: "13. A Cigánycsuk táplálékai számozatlan felsorolásban (ul, li elemekkel)",
      check: (doc) => doc.querySelector('ul#taplalek li') !== null,
    },
    {
      id: "ciganycsuk-taplalek-id",
      label: "14. A Cigánycsuk táplálékai ul egyedi azonosítója taplalek",
      check: (doc) => doc.querySelector('ul#taplalek') !== null,
    },
    {
      id: "zoldkullo-h2",
      label: "15. A Zöld küllő részben van 2-es szintű címsor",
      check: (doc) => {
        const h2s = doc.querySelectorAll('h2');
    ...
    {
      id: "zoldkullo-paragraphs",
      label: "16. A Zöld küllő szövegrészei bekezdésbe kerültek (legalább 2 p)",
      check: (doc) => {
        const h2s = doc.querySelectorAll('h2');
        const zH2 = Array.from(h2s).find(h => h.textContent.includes('Zöld küllő'));
        if (!zH2) return false;
        const container = zH2.closest('.col');
        if (!container) return false;
        return container.querySelectorAll('p:not(.card-text)').length >= 2;
      },
    },
    {
      id: "zoldkullo-img",
      label: "17. A Zöld küllő kép forrása img/zoldkullo.jpg",
      check: (doc) => doc.querySelector('img[src*="zoldkullo"]') !== null,
    },
    {
      id: "zoldkullo-alt-title",
      label: "18. A Zöld küllő képnél alt és title szövege \"Zöld küllő\"",
      check: (doc) => {
        const img = doc.querySelector('img[src*="zoldkullo"]');
        return img && img.alt === "Zöld küllő" && img.title === "Zöld küllő";
      },
    },
    {
      id: "caption-classes",
      label: "19. A képaláírásokon alkalmazza a card-text és h5 osztályjelölőket (legalább 6)",
      check: (doc) => doc.querySelectorAll('p.card-text.h5').length >= 6,
    },
    {
      id: "barkoscinege-br",
      label: "20. A Barkóscinege szövege sortörésekkel (br) van formázva",
      check: (doc) => {
        const h2s = doc.querySelectorAll('h2');
        const bH2 = Array.from(h2s).find(h => h.textContent.includes('Barkóscinege'));
        if (!bH2) return false;
        const container = bH2.closest('.col');
        return container && container.querySelector('br') !== null;
      },
    },
    {
      id: "bold-combined",
    ...
    {
      id: "table-classes",
      label: "23. A táblázaton alkalmazza a table-striped és table-warning osztályokat",
      check: (doc) => {
        const table = doc.querySelector('table');
        return table &&
    ...
    {
      id: "table-new-rows",
      label: "24. A táblázatba bekerültek az új sorok (rétisas, kerecsensólyom)",
      check: (doc) => {
        const tds = doc.querySelectorAll('tbody td');
        const text = Array.from(tds).map(td => td.textContent).join(' ');
    ...
    },
    {
      ...
      label: "25. Az Év madara program részben mindkét oszlop col-md-6 méretű",
      check: (doc) => {
    ...
    {
      id: "css-fejlec-height",
      label: "29. CSS: A fejlec azonosítóra 100%-os magasság van beállítva",
      check: (doc, html, css) => css && /#fejlec\s*\{[^}]*height\s*:\s*100%/i.test(css),
      cssCheck: true,
    },
    {
    ...
    {
      id: "css-felirat-color",
      label: "31. CSS: A felirat azonosító betűszíne rgb(13,1,81)",
      check: (doc, html, css) => css && /#felirat\s*\{[^}]*color\s*:\s*rgb\s*\(\s*13\s*,\s*1\s*,\s*81\s*\)/i.test(css),
      cssCheck: true,
    },
    {
    ...
    {
      id: "css-hobbi-bg",
      label: "33. CSS: A hobbi osztály háttérszíne #fff7ad",
    ...
    {
      id: "css-taplalek-image",
      label: "37. CSS: A taplalek azonosítóra madar.png listaelem kép van beállítva",
...
gombak: {
  ...
  checks: [
    ...
    {
      id: "title",
      label: "3. Böngésző címsorában megjelenő cím \"Gombák\"",
    ...
    {
      id: "fejlec-alt-title",
      label: "5. A fejléc kép alt és title szövege \"Gombák\"",
      check: (doc) => {
        const img = doc.querySelector('img[src*="fejlec"]');
    ...
    {
      id: "nav-4th-link",
      label: "7. Elkészítette a \"Gyógygombák\" menüpontot és jó oldalra hivatkozik",
      check: (doc) => {
        const links = doc.querySelectorAll('nav a, .navbar a');
        return Array.from(links).some(a =>
    ...
    {
      id: "nav-4th-blank",
      label: "8. A \"Gyógygombák\" link új lapon nyílik meg",
      check: (doc) => {
        const links = doc.querySelectorAll('nav a, .navbar a');
    ...
    {
      id: "muholdszoveg-paragraphs",
      label: "10. A \"Gombából műhold\" szöveg legalább 3 bekezdésbe kerültek",
      check: (doc) => {
        const erdei = doc.querySelector('#erdei');
        if (!erdei) return false;
    ...
    {
      id: "muholdszoveg-my3",
      label: "11. A \"Gombából műhold\" bekezdésekre alkalmazza a my-3 osztályjelölőt",
      check: (doc) => {
        const erdei = doc.querySelector('#erdei');
        if (!erdei) return false;
    ...
    {
      id: "laskagomba-underline",
      label: "12. Az \"5 laskagomba recept:\" szöveg aláhúzott",
      check: (doc) => {
        const underlines = doc.querySelectorAll('u');
        return Array.from(underlines).some(u => u.textContent.toLowerCase().includes('laskagomba recept'));
      },
    },
    {
      id: "laskagomba-ul",
      label: "13. A laskagomba receptek számozatlan felsorolásban (ul, li elemekkel)",
      check: (doc) => doc.querySelector('ul#kaja li') !== null,
    },
    {
      id: "laskagomba-kaja-id",
      label: "14. A laskagomba receptek ul egyedi azonosítója kaja",
      check: (doc) => doc.querySelector('ul#kaja') !== null,
    },
    {
      id: "gyilkos-galoca-h3",
      label: "15. A Gyilkos galóca részben van 3-as szintű címsor",
      check: (doc) => {
        const h3s = doc.querySelectorAll('h3');
    ...
    {
      id: "gyilkos-galoca-paragraphs",
      label: "16. A Gyilkos galóca szövegrészei bekezdésbe kerültek (legalább 2 p)",
      check: (doc) => {
        const h3s = doc.querySelectorAll('h3');
        const gH3 = Array.from(h3s).find(h => h.textContent.includes('Gyilkos galóca'));
        if (!gH3) return false;
        const container = gH3.closest('.col') || gH3.parentElement;
        return container && container.querySelectorAll('p:not(.card-text)').length >= 2;
      },
    },
    {
      id: "gyilkos-galoca-img",
      label: "17. A Gyilkos galóca kép forrása img/gyilkos_galoca.jpg",
      check: (doc) => doc.querySelector('img[src*="gyilkos_galoca"]') !== null,
    },
    {
      id: "gyilkos-galoca-alt-title",
      label: "18. A Gyilkos galóca képnél alt és title szövege \"Gyilkos galóca\"",
      check: (doc) => {
        const img = doc.querySelector('img[src*="gyilkos_galoca"]');
        return img && img.alt === "Gyilkos galóca" && img.title === "Gyilkos galóca";
      },
    },
    {
      id: "bold-combined",
      label: "19. A gombanevek félkövérek (legalább 6 helyen)",
      check: (doc) => doc.querySelectorAll('b, strong').length >= 6,
    },
    {
      id: "italic-combined",
      label: "20. A latin nevek dőltek (legalább 6 helyen)",
      check: (doc) => doc.querySelectorAll('i, em').length >= 6,
    },
    {
      id: "megelozese-col",
      label: "21. A gombamérgezés megelőzése részben az oszlopok col-md-4 és col-md-8 méretűek",
      check: (doc) => {
        const jotanacs = doc.querySelector('#jotanacs');
        if (!jotanacs) return false;
        return jotanacs.querySelector('[class*="col-md-4"]') !== null &&
          jotanacs.querySelector('[class*="col-md-8"]') !== null;
      },
    },
    {
      id: "megelozese-img-fluid",
      label: "22. A gombamérgezés megelőzése részben a kép reszponzív (img-fluid)",
      check: (doc) => {
        const jotanacs = doc.querySelector('#jotanacs');
        if (!jotanacs) return false;
        return jotanacs.querySelector('img.img-fluid') !== null;
      },
    },
    {
      id: "megelozese-ol",
      label: "23. A gombamérgezés megelőzése szöveg számozott felsorolásban (ol.fs-5)",
      check: (doc) => {
        const jotanacs = doc.querySelector('#jotanacs');
        if (!jotanacs) return false;
        return jotanacs.querySelector('ol.fs-5 li') !== null ||
          jotanacs.querySelector('ol li') !== null;
      },
    },
    {
      id: "table-th",
      label: "24. A táblázat fejlécében elkészítette az 5 fejléc cellát",
      check: (doc) => doc.querySelectorAll('thead th').length >= 5,
    },
    {
      id: "table-uppercase",
      label: "25. A táblázat fejléce nagybetűs (text-uppercase)",
      check: (doc) => {
        const thead = doc.querySelector('thead');
        return thead && (
          thead.classList.contains('text-uppercase') ||
          thead.querySelector('tr.text-uppercase') !== null ||
    ...
    {
      id: "footer-href",
      label: "26. A láblécben van hivatkozás a \"fejlec\" azonosítóra",
      check: (doc) => doc.querySelector('.lablec a[href="#fejlec"]') !== null,
    },
    {
      id: "footer-text",
      label: "27. A lábléc hivatkozás szövege \"Ugrás az elejére\"",
    ...
    {
      id: "css-body-font",
      label: "28. CSS: Az oldal betűtípusa cursive",
    ...
    {
      id: "css-h3-color",
      label: "30. CSS: A h3 betűszíne fekete",
      check: (doc, html, css) => css && /h3\s*\{[^}]*color\s*:\s*black/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-h3-fontsize",
      label: "31. CSS: A h3 betűmérete 2.2em",
      check: (doc, html, css) => css && /h3\s*\{[^}]*font-size\s*:\s*2\.2em/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-nav-border",
      label: "32. CSS: A navigáció listaelemére 3px vastag, pontozott, rgb(255,132,0) jobb szegélyt állított",
      check: (doc, html, css) => css && /nav\s+li\s*\{[^}]*border(-right)?\s*:[^;]*3px[^;]*dotted[^;]*(?:rgb\s*\(\s*255\s*,\s*132\s*,\s*0\s*\)|orange)/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-nav-padding",
      label: "33. CSS: A navigáció listaelemének vízszintes belső margója 15px",
      check: (doc, html, css) => css && /nav\s+li\s*\{[^}]*padding\s*:\s*0\s*15px/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-lablec-bold",
      label: "34. CSS: A lablec osztály hivatkozása félkövér",
      check: (doc, html, css) => css && /\.lablec\s+a\s*\{[^}]*font-weight\s*:\s*bold/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-lablec-nodecor",
      label: "35. CSS: A lablec osztály hivatkozásánál nincs aláhúzás",
      check: (doc, html, css) => css && /\.lablec\s+a\s*\{[^}]*text-decoration\s*:\s*none/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-kaja-image",
      label: "36. CSS: A kaja azonosítóra gomba.png listaelem kép van beállítva",
      check: (doc, html, css) => css && /#kaja\s*\{[^}]*list-style-image\s*:\s*url\([^)]*gomba\.png/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-kaja-margin",
      label: "37. CSS: A kaja azonosító bal külső margója 25px",
...
hobbiallatok: {
  ...
  checks: [
    ...
    {
      id: "title",
      label: "3. Böngésző címsorában megjelenő cím \"Hobbiállatok\"",
    ...
    {
      id: "fejlec-alt-title",
      label: "5. A fejléc kép alt és title szövege \"Hobbiállatok\"",
      check: (doc) => {
        const img = doc.querySelector('img[src*="fejlec"]');
    ...
    {
      id: "felirat-h1",
      label: "7. A \"Hobbiállatok\" szöveg 1-es szintű címsor (h1#felirat)",
      check: (doc) => doc.querySelector('h1#felirat') !== null,
    },
    {
      id: "nav-3rd-link",
      label: "8. Elkészítette a \"Felelős állattartás\" menüpontot",
      check: (doc) => {
        const links = doc.querySelectorAll('nav a, .navbar a');
        return Array.from(links).some(a =>
    ...
    {
      id: "nav-3rd-blank",
      label: "9. A \"Felelős állattartás\" link új lapon nyílik meg",
      check: (doc) => {
        const links = doc.querySelectorAll('nav a, .navbar a');
        return Array.from(links).some(a =>
    ...
    {
      id: "kutya-ul",
      label: "10. A kutya tulajdonságai számozatlan felsorolásban (ul)",
      check: (doc) => {
        const h3s = doc.querySelectorAll('h3');
    ...
    {
      id: "macska-ol",
      label: "11. A világ legnépszerűbb macskái számozott felsorolásban (ol)",
      check: (doc) => {
        const h3s = doc.querySelectorAll('h3');
    ...
    {
      id: "torpesun-h3",
      label: "12. A Törpesün részben van 3-as szintű címsor",
      check: (doc) => {
        const h3s = doc.querySelectorAll('h3');
    ...
    {
      id: "torpesun-paragraphs",
      label: "13. A Törpesün szövegrészei bekezdésbe kerültek (legalább 2 p)",
      check: (doc) => {
        const h3s = doc.querySelectorAll('h3');
        const tH3 = Array.from(h3s).find(h => h.textContent.includes('Törpesün'));
        if (!tH3) return false;
        const container = tH3.closest('.col') || tH3.parentElement;
        return container && container.querySelectorAll('p:not(.card-text)').length >= 2;
      },
    },
    {
      id: "torpesun-bold",
      label: "14. A Törpesün neve félkövér",
      check: (doc) => {
        const bolds = doc.querySelectorAll('b, strong');
        return Array.from(bolds).some(b => b.textContent.toLowerCase().includes('törpesün'));
      },
    },
    {
      id: "torpesun-italic",
      label: "15. A Törpesün latin neve dőlt",
      check: (doc) => {
        const italics = doc.querySelectorAll('i, em');
        return Array.from(italics).some(i => i.textContent.includes('Atelerix'));
      },
    },
    {
      id: "torpesun-img",
      label: "16. A Törpesün kép forrása img/torpesun.jpg",
      check: (doc) => doc.querySelector('img[src*="torpesun"]') !== null,
    },
    {
      id: "torpesun-alt-title",
      label: "17. A Törpesün képnél alt és title szövege \"Törpesün\"",
      check: (doc) => {
        const img = doc.querySelector('img[src*="torpesun"]');
        return img && img.alt === "Törpesün" && img.title === "Törpesün";
      },
    },
    {
      id: "caption-classes",
      label: "18. A képaláírásokon alkalmazza a fw-bold és h5 osztályjelölőket (legalább 6)",
      check: (doc) => doc.querySelectorAll('p.card-text.fw-bold.h5').length >= 6,
    },
    {
      id: "kameleon-underline",
      label: "19. A kaméleon \"Kedvenc táplálékai:\" szöveg aláhúzott",
      check: (doc) => {
        const underlines = doc.querySelectorAll('u');
    ...
    {
      id: "kameleon-eleseg-ul",
      label: "20. A kaméleon táplálékai számozatlan felsorolásban (ul#eleseg)",
      check: (doc) => doc.querySelector('ul#eleseg li') !== null,
    },
    {
      id: "tartasi-table-rows",
      label: "21. A tartási költségek táblázata elkészült (vannak adatsorok)",
      check: (doc) => {
        const koltsegek = doc.querySelector('#koltsegek');
        if (!koltsegek) return false;
        return koltsegek.querySelectorAll('tbody tr').length >= 2;
      },
    },
    {
      id: "tartasi-table-striped",
      label: "22. A tartási táblázaton alkalmazza a table-striped osztályjelölőt",
      check: (doc) => {
        const koltsegek = doc.querySelector('#koltsegek');
        if (!koltsegek) return false;
        const table = koltsegek.querySelector('table');
        return table && table.classList.contains('table-striped');
      },
    },
    {
      id: "tartasi-table-uppercase",
      label: "23. A tartási táblázat fejléce nagybetűs (text-uppercase)",
      check: (doc) => {
        const koltsegek = doc.querySelector('#koltsegek');
        if (!koltsegek) return false;
        const thead = koltsegek.querySelector('thead');
        return thead && (
          thead.classList.contains('text-uppercase') ||
          thead.querySelector('tr.text-uppercase') !== null ||
          thead.querySelector('.text-uppercase') !== null
    ...
    {
      id: "hatas-col-md",
      label: "24. Az egészségügyi hatások részben az oszlopok col-md-4 és col-md-8 méretűek",
      check: (doc) => {
    ...
    {
      id: "css-body-font",
      label: "27. CSS: Az oldal betűtípusa Verdana",
      check: (doc, html, css) => css && /body\s*\{[^}]*font-family\s*:\s*[^;]*verdana/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-fejlec-width",
      label: "28. CSS: A fejlec azonosítóra 100%-os szélesség van beállítva",
      check: (doc, html, css) => css && /#fejlec\s*\{[^}]*width\s*:\s*100%/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-h3-fontsize",
      label: "29. CSS: A h3 betűmérete 2.5em",
      check: (doc, html, css) => css && /h3\s*\{[^}]*font-size\s*:\s*2\.5em/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-hobbi-bg",
      label: "30. CSS: A hobbi osztály háttérszíne rgb(255,242,207)",
      check: (doc, html, css) => css && /\.hobbi\s*\{[^}]*background(-color)?\s*:\s*rgb\s*\(\s*255\s*,\s*242\s*,\s*207\s*\)/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-hobbi-border",
      label: "31. CSS: A hobbi osztályra 2px vastag fekete szegélyt állított",
      check: (doc, html, css) => css && /\.hobbi\s*\{[^}]*border\s*:\s*2px\s+solid\s+black/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-lablec-color",
      label: "32. CSS: A lablec osztály hivatkozásának betűszíne fekete",
      check: (doc, html, css) => css && /\.lablec\s+a\s*\{[^}]*color\s*:\s*black/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-lablec-bold",
      label: "33. CSS: A lablec osztály hivatkozása félkövér",
      check: (doc, html, css) => css && /\.lablec\s+a\s*\{[^}]*font-weight\s*:\s*bold/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-lablec-nodecor",
      label: "34. CSS: A lablec osztály hivatkozásánál nincs aláhúzás",
      check: (doc, html, css) => css && /\.lablec\s+a\s*\{[^}]*text-decoration\s*:\s*none/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-lablec-hover",
      label: "35. CSS: Ha a lablec osztály hivatkozása fölé visszük az egeret, nagybetűs",
      check: (doc, html, css) => css && /\.lablec\s+a:hover\s*\{[^}]*text-transform\s*:\s*uppercase/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-p-justify",
      label: "36. CSS: A bekezdések sorkizártak",
      check: (doc, html, css) => css && /p\s*\{[^}]*text-align\s*:\s*justify/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-eleseg-image",
      label: "37. CSS: Az eleseg azonosítóra rovar.png listaelem kép van beállítva",
      check: (doc, html, css) => css && /#eleseg\s*\{[^}]*list-style-image\s*:\s*url\([^)]*rovar\.png/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-eleseg-margin",
      label: "38. CSS: Az eleseg azonosító bal külső margója 25px",
...
hullok: {
  ...
  checks: [
    ...
    {
      id: "title",
      label: "3. Böngésző címsorában megjelenő cím \"Hüllők\"",
    ...
    {
      id: "fejlec-alt-title",
      label: "5. A fejléc kép alt és title szövege \"Hüllők\"",
      check: (doc) => {
        const img = doc.querySelector('img[src*="fejlec"]');
        return img && img.alt === "Hüllők" && img.title === "Hüllők";
      },
    },
    {
      id: "fejlec-id",
    ...
    {
      id: "felirat-h1",
      label: "7. A \"Hüllők\" szöveg 1-es szintű címsor (h1#felirat)",
      check: (doc) => doc.querySelector('h1#felirat') !== null,
    },
    {
      id: "nav-4th-link",
      label: "8. Elkészítette a \"További információ\" menüpontot",
      check: (doc) => {
        const links = doc.querySelectorAll('nav a, .navbar a');
        return Array.from(links).some(a =>
    ...
    {
      id: "nav-4th-blank",
      label: "9. A \"További információ\" link új lapon nyílik meg",
      check: (doc) => {
        const links = doc.querySelectorAll('nav a, .navbar a');
        return Array.from(links).some(a =>
    ...
    {
      id: "parduc-paragraphs",
      label: "10. A Párduc kaméleon szövegrészei bekezdésbe kerültek (legalább 2 p)",
      check: (doc) => {
        const h2s = doc.querySelectorAll('h2');
        const pardH2 = Array.from(h2s).find(h => h.textContent.toLowerCase().includes('párduc'));
        if (!pardH2) return false;
        const container = pardH2.closest('.col');
        if (!container) return false;
        return container.querySelectorAll('p:not(.card-text)').length >= 2;
      },
    },
    {
      ...
      label: "11. Az \"A gekkó színei:\" szöveg aláhúzott",
      check: (doc) => {
        const underlines = doc.querySelectorAll('u');
        return Array.from(underlines).some(u => u.textContent.includes('gekkó színei') || u.textContent.includes('A gekkó'));
      },
    },
    {
      id: "aranyporos-ul",
      label: "12. A gekkó színei számozatlan felsorolásban (ul, li elemekkel)",
      check: (doc) => doc.querySelector('ul#gyik li') !== null,
    },
    {
      id: "aranyporos-gyik-id",
      label: "13. A gekkó színei ul egyedi azonosítója gyik",
      check: (doc) => doc.querySelector('ul#gyik') !== null,
    },
    {
      id: "leguan-ol",
      label: "14. A zöld leguán táplálékai számozott felsorolásban (ol)",
      check: (doc) => {
        const h2s = doc.querySelectorAll('h2');
    ...
    {
      id: "pavaszem-paragraphs",
      label: "16. A Pávaszemes gyík szövegrészei bekezdésbe kerültek (legalább 2 p)",
      check: (doc) => {
        const h2s = doc.querySelectorAll('h2');
        const pvH2 = Array.from(h2s).find(h => h.textContent.includes('Pávaszemes gyík'));
        if (!pvH2) return false;
        const container = pvH2.closest('.col');
        if (!container) return false;
        return container.querySelectorAll('p:not(.card-text)').length >= 2;
      },
    },
    {
      id: "pavaszem-img",
      label: "17. A Pávaszemes gyík kép forrása img/pavaszem.jpg",
      check: (doc) => doc.querySelector('img[src*="pavaszem"]') !== null,
    },
    {
      id: "pavaszem-alt-title",
      label: "18. A Pávaszemes gyík képnél alt és title szövege \"Pávaszemes gyík\"",
      check: (doc) => {
        const img = doc.querySelector('img[src*="pavaszem"]');
        return img && img.alt === "Pávaszemes gyík" && img.title === "Pávaszemes gyík";
      },
    },
    {
      id: "bold-combined",
      label: "19. A hüllőnevek félkövérek (legalább 6 helyen)",
      check: (doc) => doc.querySelectorAll('b, strong').length >= 6,
    },
    {
      id: "italic-combined",
    ...
    {
      id: "table-classes",
      label: "21. A táblázaton alkalmazza a table-bordered és text-center osztályokat",
      check: (doc) => {
        const table = doc.querySelector('table');
        return table &&
          table.classList.contains('table-bordered') &&
    ...
    {
      id: "table-new-row",
      label: "22. A táblázatba bekerült a Komodói varánusz sor",
      check: (doc) => {
        const tds = doc.querySelectorAll('tbody td');
        const text = Array.from(tds).map(td => td.textContent).join(' ');
    ...
    {
      id: "erdekes-col-md-6",
      label: "23. A \"7+1 érdekes tény\" részben mindkét oszlop col-md-6 méretű",
      check: (doc) => {
    ...
    {
      id: "erdekes-img-fluid",
      label: "24. A \"7+1 érdekes tény\" részben a kép reszponzív (img-fluid)",
      check: (doc) => {
        const erdekes = doc.querySelector('#erdekes');
        if (!erdekes) return false;
        return erdekes.querySelector('img.img-fluid') !== null;
      },
    },
    {
      id: "footer-href",
    ...
    {
      id: "css-body-font",
      label: "27. CSS: Az oldal betűtípusa Rubik",
      check: (doc, html, css) => css && /body\s*\{[^}]*font-family\s*:\s*[^;]*rubik/i.test(css),
      cssCheck: true,
    },
    {
    ...
    {
      id: "css-felirat-bold",
      label: "29. CSS: A felirat azonosítóra félkövér betűstílus van beállítva",
      check: (doc, html, css) => css && /#felirat\s*\{[^}]*font-weight\s*:\s*bold/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-nav-border",
      label: "30. CSS: A navigáció listaelemére 3px vastag, pontozott, #39FF14 színű jobb szegélyt állított",
      check: (doc, html, css) => css && /nav\s+li\s*\{[^}]*border(-right)?\s*:[^;]*3px[^;]*dotted[^;]*#39FF14/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-nav-padding",
      label: "31. CSS: A navigáció listaelemének vízszintes belső margója 15px",
      check: (doc, html, css) => css && /nav\s+li\s*\{[^}]*padding\s*:\s*0\s*15px/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-dd-italic",
      label: "32. CSS: A dd elemre dőlt betűstílus van beállítva",
      check: (doc, html, css) => css && /dd\s*\{[^}]*font-style\s*:\s*italic/i.test(css),
      cssCheck: true,
    },
    {
      id: "css-dd-fontsize",
      label: "33. CSS: A dd elem betűmérete 0.9em",
    ...
    {
      id: "css-gyik-image",
      label: "37. CSS: A gyik azonosítóra gyik.png listaelem kép van beállítva",
